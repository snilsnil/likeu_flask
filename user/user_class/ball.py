...
import numpy as np
import imageio
from user.user_class.utils import score, detect_down, detect_up, in_hoop_region, clean_hoop_pos, clean_ball_pos
...
class ShotDetector():
    def __init__(self, player, csv_filename):
        ...
        self.output_path = f'user/user_output/{self.result_player}_ball.mp4'
        ...
        df.to_json(f'user/user_ball/{self.csv_filename}', orient='records', indent=4)
        print(f"공 데이터가 'user/user_ball/{self.csv_filename}'로 저장되었습니다.")
    ...
    def save_trajectory_image(self):
        # 궤적 이미지 저장
        trajectory_path = f"user/user_output/{self.result_player}_trajectory.png"
        cv2.imwrite(trajectory_path, self.trajectory_image)
        print(f"공의 궤적 이미지가 {trajectory_path}에 저장되었습니다.")
        
        # GIF 저장
        gif_path = f"user/user_output/{self.result_player}_trajectory.gif"
