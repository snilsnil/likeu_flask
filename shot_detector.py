...
class ShotDetector():
    ...
    def clean_motion(self):
        ...
        # 임계값 설정 (너무 큰 이동을 무시)
        distance_threshold = 70  # 픽셀 거리 이거 해상도애 따라 다른데 폰이면 100정도 하면됨
    ...
    def save_trajectory_image(self):
        # 궤적 이미지 저장
        trajectory_path = f"./test_output/{self.player}_trajectory.png"
        cv2.imwrite(trajectory_path, self.trajectory_image)
        print(f"공의 궤적 이미지가 {trajectory_path}에 저장되었습니다.")
        
        # GIF 저장
        gif_path = f"./test_output/{self.player}_trajectory.gif"
